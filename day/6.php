<?
function decompose($instruction) {
    $transform = array();
...
function apply_switches($grid, $corner, $transform) {
    $gx = $corner[0] - 1;
    $gy = $corner[1] - 1;
...
function apply_brightness($grid, $corner, $transform) {
    $gx = $corner[0] - 1;
    $gy = $corner[1] - 1;
...
$day = array(
    ...
    'solve' => function($input) {
        ...
        foreach (preg_split('/\n/', $input) as $line) {
            ...
            $rule = decompose($line);
            $grid1 = apply_switches($grid1, $rule[0], $rule[1]);
            $grid2 = apply_brightness($grid2, $rule[0], $rule[1]);
        };
